...
function calculateIdealBrakingForce() {
  ...
  let resultHTML = "<h3>理想制動力 計算結果</h3><p>Fr理想制動力=μg（Fr軸重 + μ・車重・重心高/ホイールベース)</p><p>Rr理想制動力=μg（Rr軸重 - μ・車重・重心高/ホイールベース)</p>";
...
function calculateActualBrakingForce() {
  ...
  let resultHTML = "<h3>実制動力 計算結果</h3><p>実制動力=2・BEF・液圧・シリンダ面積・制動有効半径/タイヤ動荷重半径</p>";
...
function drawGraph(FrBrakingForces, RrBrakingForces, FrActualBrakingForces, RrActualBrakingForces) {
  let ctx = document.getElementById("myChart"); // キャンバス要素を取得
  new Chart(ctx, {
      ...
      datasets: [
        {
          ...
          data: FrBrakingForces.map((value, index) => ({ x: value, y: RrBrakingForces[index] })), // 理想制動力のデータ(x軸をfr制動力、y軸をRr制動力に)
        ...
        },
        {
          ...
          data: FrActualBrakingForces.map((value, index) => ({ x: value, y: RrActualBrakingForces[index] })), // 実制動力のデータ(x軸をfr制動力、y軸をRr制動力に)
      ...
      scales: {
        yAxes: [{
          ...
          scaleLabel: {
            ...
            labelString: 'Rr制動力(N)' // ラベルテキスト
          },
          ticks: {
            beginAtZero: true, // 0から開始
            suggestedMax: 10000, // 最大値
